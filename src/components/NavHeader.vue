  ...
  <header class="header">
    ...
    <div class="navbar">
      ...
      <div class="navbar-right-container" style="display: flex;">
        ...
        <!-- <router-link class="btn btn--m btn--red" href="javascript:;" to="/zhuce">注册</router-link> -->
...
export default {
  ...
  methods: {
   search(){
      this.$router.push({ path: "/beizi" });
    },
    checkLogin() {
      axios.get("/api/users/checkLogin").then(response => {
        var res = response.data;
        var path = this.$route.pathname; // eslint-disable-line no-unused-vars
        if (res.status == "0") {
